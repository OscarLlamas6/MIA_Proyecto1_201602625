...
//EjecutarChgrp function
func EjecutarChgrp(name string, grupo string, id string) {

	if sesionRoot {

		if name != "" && grupo != "" && id != "" {

			if len(name) <= 10 && len(grupo) <= 10 {
				...
				if IDYaRegistrado(id) {
					...
					if name != "root" {
							...
							if GrupoYaExiste := ExisteGrupo(grupo, id); GrupoYaExiste {
								...
								if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
									...
									if SB1.MontajesCount > 0 {
											...
											if registro[1] == "U" && registro[0] != "0" {
												if registro[3] == name {
													...
													if ContenidoAux == "" {
														...
														ContenidoAux += registro[0] + "," + registro[1] + "," + grupo + "," + registro[3] + "," + registro[4]
										...
										ContenidoChgrp := name + "," + grupo
										var ContenidoChars [300]byte
										copy(ContenidoChars[:], ContenidoChgrp)
										...
										color.Printf("@{!m}El usuario @{!y}%v @{!m}fue cambiado exitosamente al grupo @{!y}%v\n", name, grupo)

										//EN ESTE MOMENTO EL USUARIO HA SIDO ELIMINADO CORRECTAMENTE, SIN EMBARGO,
										//DEBEMOS ACTUALIZAR EL GRUPO DE TODAS AQUELLAS CARPETAS Y ARCHIVOS CUYO PROPIETARIO CAMBIO DE GRUPO
										...
										CambiarPropietarioAVDRecursivo(fileMBR, ApuntadorAVD, &AVDroot, name, name, grupo)
										fileMBR.Sync()
								...
								color.Printf("@{!r}No existe ningun grupo registrado con el nombre @{!y}%v\n", grupo)
							}
						...
						color.Printf("@{!r}El sistema no permite cambiar de grupo al usuario @{!y}%v\n", name)
					}
				...
				color.Println("@{!r}	El username y el groupname no pueden tener más de 10 caracteres.")
			}
		...
		color.Println("@{!r}Se necesita de una sesión root activa para ejecutar la función CHGRP.")
	}
