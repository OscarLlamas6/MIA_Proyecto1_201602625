		...
		if IDYaRegistrado(id) {
			...
			if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
				...
				if SB1.MontajesCount > 0 {
					...
					SB1.FreeBitacoras = SB1.TotalBitacoras - int32(2)                 //Restamos porque al crear la carpeta Root y el archivo Users.txt ocupamos 2 bitacoras
					...
					color.Println("@{!c} Simulación LOSS realizada con exito.")
