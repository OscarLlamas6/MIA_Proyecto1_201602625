...
//EjecutarMkgrp inicia la creación de un nuevo grupo
func EjecutarMkgrp(grupo string, id string) {
		...
		if grupo != "" && id != "" {
			...
			if len(grupo) <= 10 {
				...
				if GrupoYaExiste := ExisteGrupo(grupo, id); !GrupoYaExiste {
					...
					if IDYaRegistrado(id) {
						...
						if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
							...
							if SB1.MontajesCount > 0 {
								...
								GroupID := fmt.Sprintf(`%v`, NumeroGrupos(id)+1)
								CadenaContenido += "\n" + GroupID + ",G" + "," + grupo
								...
								ContenidoMkgroup := grupo
								var ContenidoChars [300]byte
								copy(ContenidoChars[:], ContenidoMkgroup)
								...
								color.Printf("@{!m}El grupo @{!y}%v @{!m}fue creado con éxito\n", grupo)
					...
					color.Printf("@{!r}Ya hay un grupo registrado con el nombre @{!y}%v\n", grupo)
				}

			} else {
				color.Println("@{!r}	El nombre del grupo no puede tener más de 10 caracteres.")
			}

		} else {
			color.Println("@{!r}	Faltan parámetros obligatorios para la función Mkgrp.")
		}

	} else {
		color.Println("@{!r}Se necesita de una sesión root activa para ejecutar la función MKGRP.")
	}

}

//NumeroGrupos devuelve la cantidad de usuarios existentes
func NumeroGrupos(id string) int {
