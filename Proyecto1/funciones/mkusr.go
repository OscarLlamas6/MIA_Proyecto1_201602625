...
func EjecutarMkusr(name string, pass string, grupo string, id string) {
	...
	if sesionRoot {
		...
		if name != "" && pass != "" && id != "" && grupo != "" {
			...
			if len(name) <= 10 && len(pass) <= 10 && len(grupo) <= 10 {
				...
				if UsuarioYaExiste := ExisteUsuario(name, id); !UsuarioYaExiste {
					...
					if GrupoYaExiste := ExisteGrupo(grupo, id); GrupoYaExiste {
						...
						color.Println("@{!r}	El grupo especificado no existe en el sistema LWH.")
					}
			...
			color.Println("@{!r}	Faltan parámetros obligatorios para la función Mkusr.")
		}

	} else {
		color.Println("@{!r}Se necesita de una sesión root activa para ejecutar la función MKUSR.")
	}
