...
//EjecutarRmgrp inicia la creación de un nuevo grupo
func EjecutarRmgrp(name string, id string) {
	...
	if sesionRoot {
		...
		if name != "" && id != "" {
			...
			if len(name) <= 10 {
				...
				if IDYaRegistrado(id) {
					...
					if name != "root" {
						...
						if GrupoYaExiste := ExisteGrupo(name, id); GrupoYaExiste {
							...
							if Existe, Indice := ExisteParticion(PathAux, NameAux); Existe {
								...
								if SB1.MontajesCount > 0 {
										...
										if registro[1] == "G" && registro[0] != "0" {
											if registro[2] == name {
												...
												if ContenidoAux == "" {
													...
													ContenidoAux += "0" + "," + registro[1] + "," + registro[2]
												} else {
													ContenidoAux += "\n"
													ContenidoAux += "0" + "," + registro[1] + "," + registro[2]
												}
									...
									copy(ContenidoChars[:], ContenidoRmgrp)
									...
									color.Printf("@{!m}El grupo @{!y}%v @{!m}fue eliminado con éxito\n", name)
						...
						color.Printf("@{!r}El sistema no permite eliminar el usuario @{!y}%v\n", name)
					}
			...
			color.Println("@{!r}	Faltan parámetros obligatorios para la función Rmgrp.")
		}
